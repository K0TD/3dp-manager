...
export class RoutingPresetsService {
  ...
  private view(
    ...
    try {
      const plan = buildRoutingPlan(
        node.id,
        { template: snapshot.template, inbounds: snapshot.inbounds },
        state,
        state,
      );
    ...
    if (state.pending)
      warnings.push(
        'Предыдущая операция не завершена. Проверьте настройки и работу Xray; повторное применение восстановит выбранные пресеты, если нет ручных конфликтов.',
      );
    if (needsApply)
      warnings.push(
  ...
  async update(
    ...
    return this.store.withLock(id, async () => {
      ...
      if (
        routingRevision(before.template, before.inbounds, previous) !==
        dto.revision
      ) {
        throw new ConflictException(
      ...
      const plan = buildRoutingPlan(
        id,
        { template: before.template, inbounds: before.inbounds },
        previous,
        dto,
      );
      ...
      if (dto.blockRussia) {
        try {
          if (
            !(await this.xui.validateRoutingGeodata(node, before.template.path))
          )
            warnings.push(
      ...
      if (
        routingRevision(fresh.template, fresh.inbounds, previous) !==
        dto.revision
      ) {
        throw new ConflictException(
          'Конфигурация ноды изменилась во время проверки. Обновите состояние.',
        );
      }
      const { pendingPrevious: _older, ...recoveryState } = previous;
      await this.store.save(id, {
        ...
        pendingPrevious: {
          ...
          ...recoveryState,
          sniffing: {
            ...previous.pendingPrevious?.sniffing,
            ...previous.sniffing,
          },
      ...
      try {
        ...
        for (const update of plan.inboundUpdates) {
          const current = (await this.xui.listRoutingInbounds(node)).find(
            (i) => i.id === update.id,
          );
          ...
          if (
            ...
            throw new ConflictException(
              'Inbound изменился во время применения.',
            );
          }
        ...
        if (
          buildRoutingPlan(
            id,
            { template: saved.template, inbounds: saved.inbounds },
            plan.state,
            dto,
          ).changed
        ) {
        ...
        const message = restored
          ? 'Применение не удалось. Прежние настройки восстановлены.'
          : result === 'unknown'
        ...
        return {
          available: true,
          blockRussia: restored ? previous.blockRussia : plan.state.blockRussia,
          blockIpCheckers: restored
            ? previous.blockIpCheckers
            : plan.state.blockIpCheckers,
          revision: '',
          needsApply: true,
  ...
  private async rollback(
    ...
    attemptedInbounds: XuiInboundRaw[],
  ) {
    ...
    for (const applied of [...attemptedInbounds].reverse()) {
      ...
      if (!inbound || inboundIdentity(inbound) !== inboundIdentity(original))
      ...
      for (const key of new Set([
        ...Object.keys(oldSniffing),
        ...Object.keys(appliedSniffing),
      ])) {
        undoField(sniffing, key, oldSniffing[key], appliedSniffing[key]);
      }
      if (!equal(sniffing, sniffingObject(inbound)))
        await this.xui.updateInboundSniffing(node, {
          ...inbound,
    ...
    if (templateAttempted) {
      ...
      undoField(
        routing,
        'domainStrategy',
        oldRouting.domainStrategy,
        appliedRouting.domainStrategy,
      );
      if (
        Object.keys(routing).length ||
        before.template.config.routing !== undefined
      )
      ...
      undoField(
        config,
        'outbounds',
        before.template.config.outbounds,
        plan.template.config.outbounds,
      );
      if (!equal(config, current.template.config))
        await this.xui.saveXrayTemplate(node, { ...current.template, config });
    }
    await this.xui.applyXrayConfig(node);
    ...
    if (templateAttempted) {
      for (const key of ['outbounds', 'routing']) {
        if (!equal(restored.template.config[key], before.template.config[key]))
          throw new XuiApiError('Не удалось подтвердить откат маршрутизации');
      }
    }
    for (const applied of attemptedInbounds) {
      const original = before.inbounds.find((i) => i.id === applied.id);
      const actual = restored.inbounds.find((i) => i.id === applied.id);
      if (!actual || !equal(sniffingObject(actual), sniffingObject(original)))
        throw new XuiApiError('Не удалось подтвердить откат sniffing');
    }
  }
}
